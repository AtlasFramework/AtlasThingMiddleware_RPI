...
//ahmed added this
char *output_bufferModified(FILE * stream, uint8_t * buffer, int length, int indent){

    int p = 0;  //iteration number
    int i;
    char *result = malloc(32);
    char *finalResult = malloc(32);
    int index = 0;
    ...
    int found = 0; //we need to drop the "00-" that starts for some reason the response back from the server 
    int start = 0;
    int nstart = 0;
    for(start = 0 ; start < strlen(result) ; start++){
      	if(found == 1){
             	finalResult[nstart] = result[start];
		nstart +=1;
      	}
	if(result[start] == '-'){
		found = 1;
	}
	if (!isprint(result[start])){
		break;
	}
    }
